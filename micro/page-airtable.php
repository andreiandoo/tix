...
$t = [
	...
	// Testimonial
	'testimonial'         => $current_lang === 'ro' ? 'Echipa folosea deja <span class="font-semibold text-gradient-airtable">Airtable</span> pentru planificare. Acum vanzarile de bilete apar automat langa task-uri. Toata lumea vede imaginea completa.' : 'The team was already using <span class="font-semibold text-gradient-airtable">Airtable</span> for planning. Now ticket sales appear automatically next to tasks. Everyone sees the complete picture.',
...
<main class="overflow-x-hidden font-body bg-dark-900 text-zinc-200">
  ...
  <section class="relative flex items-center min-h-screen pt-20 overflow-hidden">
    ...
    <div class="px-6 py-20 mx-auto max-w-7xl lg:px-8">
      <div class="grid items-center gap-12 lg:grid-cols-2 lg:gap-16">
        ...
        <div class="reveal">
          <!-- Badge -->
          <div class="inline-flex items-center gap-3 px-4 py-2 mb-6 border rounded-full bg-airtable-primary/10 border-airtable-primary/20">
            ...
            <span class="text-sm font-medium text-airtable-primary"><?php echo esc_html( $t['badge'] ); ?></span>
          </div>
          ...
          <div class="flex flex-wrap gap-4 mb-12">
            <a href="<?php echo esc_url(home_url('/signup')); ?>" class="inline-flex items-center gap-2 px-8 py-4 font-semibold transition-all duration-300 rounded-full group bg-airtable-primary text-airtable-dark hover:bg-airtable-secondary hover:text-white hover:scale-105 hover:shadow-glow-airtable">
          ...
          <div class="flex flex-wrap items-center gap-4">
            <div class="flex items-center gap-2">
              ...
              <span class="text-sm text-white/60">Bidirectional Sync</span>
            </div>
            <div class="flex items-center gap-2">
            ...
            <div class="flex items-center gap-2">
              ...
              <span class="text-sm text-white/60">OAuth 2.0</span>
            </div>
          </div>
        </div>
        ...
        <div class="reveal reveal-delay-1">
          <div class="relative" x-data="{
            ...
            <!-- Airtable Table -->
            <div class="shadow-2xl airtable-table">
              <!-- Table Header -->
              <div class="flex items-center justify-between px-4 py-3 airtable-header">
                <div class="flex items-center gap-3">
                  <div class="flex items-center justify-center w-8 h-8 rounded-lg bg-gradient-to-br from-airtable-primary to-airtable-secondary">
                  ...
                  <div>
                    <div class="text-sm font-semibold text-white"><?php echo esc_html( $t['festival_orders'] ); ?></div>
                    <div class="text-xs text-white/40">appXyz123 • 3 <?php echo esc_html( $t['records'] ); ?></div>
                  </div>
                </div>
                <div class="flex items-center gap-2">
              ...
              <div class="grid grid-cols-4 airtable-header">
                <div class="text-sm font-semibold airtable-cell text-white/70">
                  <span class="mr-2 field-type text">Text</span>
                  <?php echo esc_html( $t['order'] ); ?>
                </div>
                <div class="text-sm font-semibold airtable-cell text-white/70">
                  <span class="mr-2 field-type text">Email</span>
                  <?php echo esc_html( $t['customer'] ); ?>
                </div>
                <div class="text-sm font-semibold airtable-cell text-white/70">
                  <span class="mr-2 field-type currency">€</span>
                  Total
                </div>
                <div class="text-sm font-semibold airtable-cell text-white/70">
                  <span class="mr-2 field-type select">⬤</span>
                  Status
                </div>
              </div>
              ...
              <template x-for="(row, index) in rows" :key="row.order">
                <div class="grid grid-cols-4 airtable-row animate-row-slide" :style="'animation-delay: ' + (index * 0.1) + 's'">
                  <div class="font-mono text-sm text-white airtable-cell" x-text="row.order"></div>
                  <div class="text-sm airtable-cell text-white/70" x-text="row.email"></div>
                  <div class="font-semibold airtable-cell text-airtable-primary" x-text="row.total"></div>
              ...
              <!-- New Row Indicator -->
              <div class="grid grid-cols-4 opacity-50 airtable-row" x-show="syncing">
                <div class="font-mono text-sm airtable-cell text-white/30">ORD-004</div>
                <div class="text-sm airtable-cell text-white/30">nou@client.ro</div>
                <div class="airtable-cell text-white/30">€100</div>
                <div class="text-xs airtable-cell text-white/30">Sincronizare...</div>
              </div>
            </div>

            <!-- Floating Sync Badge -->
            <div class="absolute z-10 px-4 py-3 border shadow-xl -top-4 -right-4 bg-dark-800 rounded-xl border-airtable-accent/30 animate-float">
              <div class="flex items-center gap-2">
                ...
                <div>
                  <div class="text-sm font-medium text-airtable-accent"><?php echo esc_html( $t['bidirectional'] ); ?></div>
                  <div class="text-xs text-white/40"><?php echo esc_html( $t['realtime_sync'] ); ?></div>
                </div>
              </div>
            </div>
            ...
            <div class="absolute -bottom-4 -left-4 bg-dark-800 rounded-xl px-4 py-3 border border-airtable-green/30 shadow-xl animate-float [animation-delay:1s] z-10">
              <div class="flex items-center gap-2">
                <svg class="w-5 h-5 text-airtable-green" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
                <div>
                  <div class="text-sm font-medium text-airtable-green">OAuth 2.0</div>
                  <div class="text-xs text-white/40"><?php echo esc_html( $t['secure_auth'] ); ?></div>
                </div>
              </div>
            </div>
          </div>
  ...
  <section class="relative py-24 overflow-hidden" id="functionalitati">
    <div class="px-6 mx-auto max-w-7xl lg:px-8">
      ...
      <div class="max-w-3xl mx-auto mb-16 text-center reveal">
        <span class="text-sm font-medium tracking-widest uppercase text-airtable-primary"><?php echo esc_html( $t['sync_badge'] ); ?></span>
        <h2 class="mt-4 mb-6 text-4xl font-bold text-white font-display md:text-5xl"><?php echo esc_html( $t['sync_title'] ); ?><br><span class="text-gradient-airtable"><?php echo esc_html( $t['sync_title2'] ); ?></span></h2>
      ...
      <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-4 reveal">
        <!-- Push Only -->
        <div class="relative p-6 transition-all duration-500 border feature-card bg-dark-800/50 rounded-2xl border-airtable-primary/20 hover:border-airtable-primary/50">
          <div class="flex items-center justify-center mb-4 w-14 h-14 rounded-2xl bg-airtable-primary/10">
          ...
          <h3 class="mb-2 text-xl font-semibold text-white"><?php echo esc_html( $t['push_only'] ); ?></h3>
          <p class="mb-4 text-sm text-white/50"><?php echo esc_html( $t['push_desc'] ); ?></p>
          <span class="sync-badge push">Tixello → Airtable</span>
        </div>

        <!-- Bidirectional -->
        <div class="relative p-6 transition-all duration-500 border feature-card bg-dark-800/50 rounded-2xl border-airtable-accent/20 hover:border-airtable-accent/50">
          <div class="flex items-center justify-center mb-4 w-14 h-14 rounded-2xl bg-airtable-accent/10">
          ...
          <h3 class="mb-2 text-xl font-semibold text-white"><?php echo esc_html( $t['bidirectional'] ); ?></h3>
          <p class="mb-4 text-sm text-white/50"><?php echo esc_html( $t['bidirectional_desc'] ); ?></p>
          <span class="sync-badge bidirectional">↔ Two-way Sync</span>
        </div>

        <!-- Scheduled -->
        <div class="relative p-6 transition-all duration-500 border feature-card bg-dark-800/50 rounded-2xl border-airtable-purple/20 hover:border-airtable-purple/50">
          <div class="flex items-center justify-center mb-4 w-14 h-14 rounded-2xl bg-airtable-purple/10">
          ...
          <h3 class="mb-2 text-xl font-semibold text-white"><?php echo esc_html( $t['scheduled'] ); ?></h3>
          <p class="mb-4 text-sm text-white/50"><?php echo esc_html( $t['scheduled_desc'] ); ?></p>
          <span class="px-3 py-1 text-xs rounded bg-airtable-purple/20 text-airtable-purple">⏱ <?php echo esc_html( $t['hourly_daily'] ); ?></span>
        </div>

        <!-- Real-time -->
        <div class="relative p-6 transition-all duration-500 border feature-card bg-dark-800/50 rounded-2xl border-airtable-green/20 hover:border-airtable-green/50">
          <div class="flex items-center justify-center mb-4 w-14 h-14 rounded-2xl bg-airtable-green/10">
          ...
          <h3 class="mb-2 text-xl font-semibold text-white"><?php echo esc_html( $t['realtime'] ); ?></h3>
          <p class="mb-4 text-sm text-white/50"><?php echo esc_html( $t['realtime_desc'] ); ?></p>
  ...
  <section class="relative py-24 bg-dark-850">
    <div class="px-6 mx-auto max-w-7xl lg:px-8">
      <div class="grid items-center gap-16 lg:grid-cols-2">
        <!-- Content -->
        <div class="reveal">
          <span class="text-sm font-medium tracking-widest uppercase text-airtable-accent"><?php echo esc_html( $t['field_badge'] ); ?></span>
          <h2 class="mt-4 mb-6 text-4xl font-bold text-white font-display md:text-5xl"><?php echo esc_html( $t['field_title'] ); ?><br><span class="text-gradient-airtable"><?php echo esc_html( $t['field_title2'] ); ?></span></h2>
          <p class="mb-8 text-lg text-white/60"><?php echo esc_html( $t['field_desc'] ); ?></p>

          <div class="space-y-4">
            <div class="flex items-center gap-4 p-4 border rounded-xl bg-airtable-primary/10 border-airtable-primary/30">
        ...
        <div class="reveal reveal-delay-1">
          <div class="p-6 border bg-dark-800 rounded-2xl border-white/10">
            <div class="flex items-center justify-between mb-6">
              <div class="font-semibold text-white"><?php echo esc_html( $t['field_badge'] ); ?></div>
              <span class="text-xs text-airtable-green">✓ <?php echo esc_html( $t['fields_mapped'] ); ?></span>
            </div>

            <!-- Mapping Grid -->
            <div class="space-y-4">
              <!-- Header -->
              <div class="grid grid-cols-5 gap-4 text-xs uppercase text-white/40">
              ...
              <!-- Mappings -->
              <div class="grid items-center grid-cols-5 gap-4 p-3 rounded-lg bg-dark-900/50">
                <div class="flex items-center col-span-2 gap-2">
                  <div class="w-3 h-3 rounded bg-brand-violet"></div>
                  <span class="text-sm text-white">order_number</span>
                </div>
                ...
                <div class="flex items-center col-span-2 gap-2">
                  ...
                  <span class="text-sm text-white">Nr. Comanda</span>
                  <span class="ml-auto field-type text">Text</span>
                </div>
              </div>
              ...
              <div class="grid items-center grid-cols-5 gap-4 p-3 rounded-lg bg-dark-900/50">
                <div class="flex items-center col-span-2 gap-2">
                  <div class="w-3 h-3 rounded bg-brand-violet"></div>
                  <span class="text-sm text-white">customer_email</span>
                </div>
                <div class="flex justify-center col-span-1">
                  <div class="w-full mapping-line"></div>
                </div>
                <div class="flex items-center col-span-2 gap-2">
                  <div class="w-3 h-3 rounded bg-airtable-primary"></div>
                  <span class="text-sm text-white">Email Client</span>
                  <span class="ml-auto field-type text">Email</span>
                </div>
              </div>
              ...
              <div class="grid items-center grid-cols-5 gap-4 p-3 rounded-lg bg-dark-900/50">
                <div class="flex items-center col-span-2 gap-2">
                  <div class="w-3 h-3 rounded bg-brand-violet"></div>
                  <span class="text-sm text-white">total_amount</span>
                </div>
                <div class="flex justify-center col-span-1">
                  <div class="w-full mapping-line"></div>
                </div>
                <div class="flex items-center col-span-2 gap-2">
                  ...
                  <span class="ml-auto field-type currency">€</span>
                </div>
              </div>
              ...
              <div class="grid items-center grid-cols-5 gap-4 p-3 rounded-lg bg-dark-900/50">
                <div class="flex items-center col-span-2 gap-2">
                  <div class="w-3 h-3 rounded bg-brand-violet"></div>
                  <span class="text-sm text-white">payment_status</span>
                </div>
                <div class="flex justify-center col-span-1">
                  <div class="w-full mapping-line"></div>
                </div>
                <div class="flex items-center col-span-2 gap-2">
                  ...
                  <span class="ml-auto field-type select">⬤</span>
                </div>
              </div>
              ...
              <div class="grid items-center grid-cols-5 gap-4 p-3 rounded-lg bg-dark-900/50">
                <div class="flex items-center col-span-2 gap-2">
                  <div class="w-3 h-3 rounded bg-brand-violet"></div>
                  <span class="text-sm text-white">created_at</span>
                </div>
                <div class="flex justify-center col-span-1">
                  <div class="w-full mapping-line"></div>
                </div>
                <div class="flex items-center col-span-2 gap-2">
                  <div class="w-3 h-3 rounded bg-airtable-primary"></div>
                  <span class="text-sm text-white">Data</span>
                  <span class="ml-auto field-type date">📅</span>
                </div>
              </div>

              <!-- Add Field -->
              <button class="w-full p-3 text-sm transition-colors border-2 border-dashed rounded-lg border-white/20 text-white/40 hover:border-airtable-primary/50 hover:text-airtable-primary">
  ...
  <section class="relative py-24 overflow-hidden">
    <div class="px-6 mx-auto max-w-7xl lg:px-8">
      <div class="max-w-3xl mx-auto mb-16 text-center reveal">
        <span class="text-sm font-medium tracking-widest uppercase text-airtable-secondary"><?php echo esc_html( $t['org_badge'] ); ?></span>
        <h2 class="mt-4 mb-6 text-4xl font-bold text-white font-display md:text-5xl"><?php echo esc_html( $t['bases_title'] ); ?><br><span class="text-gradient-airtable"><?php echo esc_html( $t['bases_title2'] ); ?></span></h2>
        <p class="text-lg text-white/60"><?php echo esc_html( $t['bases_desc'] ); ?></p>
      </div>

      <!-- Bases Grid -->
      <div class="grid gap-6 md:grid-cols-3 reveal">
        <!-- Sales Base -->
        <div class="base-card">
          <div class="flex items-center gap-3 mb-4">
            <div class="flex items-center justify-center w-10 h-10 rounded-lg bg-gradient-to-br from-airtable-primary to-airtable-secondary">
              <svg class="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            </div>
            <div>
              <div class="font-semibold text-white"><?php echo esc_html( $t['ticket_sales'] ); ?></div>
              <div class="text-xs text-white/40">appSales123</div>
            </div>
          </div>
        ...
        <div class="base-card">
          <div class="flex items-center gap-3 mb-4">
            <div class="flex items-center justify-center w-10 h-10 rounded-lg bg-gradient-to-br from-airtable-accent to-airtable-green">
            ...
            <div>
              <div class="font-semibold text-white"><?php echo esc_html( $t['crm_customers'] ); ?></div>
              <div class="text-xs text-white/40">appCRM456</div>
            </div>
          </div>
          <div class="space-y-2 text-sm">
            <div class="flex items-center justify-between text-white/60">
        ...
        <div class="base-card">
          <div class="flex items-center gap-3 mb-4">
            <div class="flex items-center justify-center w-10 h-10 rounded-lg bg-gradient-to-br from-airtable-purple to-airtable-red">
            ...
            <div>
              <div class="font-semibold text-white"><?php echo esc_html( $t['planning'] ); ?></div>
              <div class="text-xs text-white/40">appPlan789</div>
            </div>
          </div>
          <div class="space-y-2 text-sm">
            <div class="flex items-center justify-between text-white/60">
      ...
      <div class="max-w-2xl mx-auto mt-12 reveal reveal-delay-1">
        <div class="flex items-center gap-4 p-4 border rounded-xl bg-airtable-accent/10 border-airtable-accent/30">
          <div class="flex items-center justify-center flex-shrink-0 w-12 h-12 rounded-xl bg-airtable-accent/20">
          ...
          <div>
            <div class="font-medium text-airtable-accent"><?php echo esc_html( $t['linked_records'] ); ?></div>
            <div class="text-sm text-white/60"><?php echo esc_html( $t['linked_desc'] ); ?></div>
          </div>
        </div>
      </div>
    </div>
  ...
  <section class="relative py-24 bg-dark-850">
    <div class="px-6 mx-auto max-w-7xl lg:px-8">
      ...
      <div class="grid gap-6 md:grid-cols-2">
        <!-- Event Planning -->
        <div class="relative p-6 transition-all duration-500 border feature-card bg-dark-800/50 rounded-2xl border-white/10 hover:border-airtable-primary/30 reveal">
          <div class="flex items-start gap-4">
            <div class="flex items-center justify-center flex-shrink-0 w-14 h-14 rounded-2xl bg-gradient-to-br from-airtable-primary/20 to-airtable-primary/10"><span class="text-2xl">📅</span></div>
            <div>
              <h3 class="mb-2 text-xl font-semibold text-white"><?php echo esc_html( $t['event_planning'] ); ?></h3>
              <p class="mb-4 text-sm text-white/50"><?php echo esc_html( $t['event_planning_desc'] ); ?></p>
              <div class="flex flex-wrap gap-2">
                <span class="px-2 py-1 text-xs rounded bg-airtable-primary/20 text-airtable-primary">Kanban</span>
                <span class="px-2 py-1 text-xs rounded bg-airtable-primary/20 text-airtable-primary">Calendar</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <!-- Customer Success -->
        <div class="relative p-6 transition-all duration-500 border feature-card bg-dark-800/50 rounded-2xl border-white/10 hover:border-airtable-accent/30 reveal reveal-delay-1">
          <div class="flex items-start gap-4">
            <div class="flex items-center justify-center flex-shrink-0 w-14 h-14 rounded-2xl bg-gradient-to-br from-airtable-accent/20 to-airtable-accent/10"><span class="text-2xl">🤝</span></div>
            <div>
              <h3 class="mb-2 text-xl font-semibold text-white"><?php echo esc_html( $t['customer_success'] ); ?></h3>
              <p class="mb-4 text-sm text-white/50"><?php echo esc_html( $t['customer_success_desc'] ); ?></p>
              <div class="flex flex-wrap gap-2">
                <span class="px-2 py-1 text-xs rounded bg-airtable-accent/20 text-airtable-accent">CRM</span>
                <span class="px-2 py-1 text-xs rounded bg-airtable-accent/20 text-airtable-accent">Follow-ups</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="relative p-6 transition-all duration-500 border feature-card bg-dark-800/50 rounded-2xl border-white/10 hover:border-airtable-green/30 reveal">
          <div class="flex items-start gap-4">
            <div class="flex items-center justify-center flex-shrink-0 w-14 h-14 rounded-2xl bg-gradient-to-br from-airtable-green/20 to-airtable-green/10"><span class="text-2xl">📊</span></div>
            <div>
              <h3 class="mb-2 text-xl font-semibold text-white"><?php echo esc_html( $t['ops_dashboard'] ); ?></h3>
              <p class="mb-4 text-sm text-white/50"><?php echo esc_html( $t['ops_dashboard_desc'] ); ?></p>
              <div class="flex flex-wrap gap-2">
                <span class="px-2 py-1 text-xs rounded bg-airtable-green/20 text-airtable-green">Vizualizari</span>
                <span class="px-2 py-1 text-xs rounded bg-airtable-green/20 text-airtable-green">Filtre</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <!-- Team Collaboration -->
        <div class="relative p-6 transition-all duration-500 border feature-card bg-dark-800/50 rounded-2xl border-white/10 hover:border-airtable-purple/30 reveal reveal-delay-1">
          <div class="flex items-start gap-4">
            <div class="flex items-center justify-center flex-shrink-0 w-14 h-14 rounded-2xl bg-gradient-to-br from-airtable-purple/20 to-airtable-purple/10"><span class="text-2xl">👥</span></div>
            <div>
              <h3 class="mb-2 text-xl font-semibold text-white"><?php echo esc_html( $t['team_collab'] ); ?></h3>
              <p class="mb-4 text-sm text-white/50"><?php echo esc_html( $t['team_collab_desc'] ); ?></p>
              <div class="flex flex-wrap gap-2">
                <span class="px-2 py-1 text-xs rounded bg-airtable-purple/20 text-airtable-purple">Sharing</span>
                <span class="px-2 py-1 text-xs rounded bg-airtable-purple/20 text-airtable-purple">Permissions</span>
              </div>
            </div>
          </div>
        </div>
  ...
  <section class="relative py-24">
    <div class="max-w-4xl px-6 mx-auto lg:px-8">
      <div class="relative reveal">
        ...
        <div class="p-8 border bg-gradient-to-br from-airtable-primary/10 to-airtable-secondary/10 rounded-3xl md:p-12 border-airtable-primary/20">
          ...
          <div class="flex items-center gap-4">
            <div class="flex items-center justify-center rounded-full w-14 h-14 bg-gradient-to-br from-airtable-primary to-airtable-secondary">
              <span class="font-bold text-airtable-dark">ML</span>
            </div>
            <div>
  ...
  <section class="relative py-32 overflow-hidden">
    ...
    <div class="relative max-w-4xl px-6 mx-auto text-center lg:px-8">
      <h2 class="mb-6 text-5xl font-bold text-white font-display md:text-7xl reveal"><?php echo esc_html( $t['final_title'] ); ?><br><span class="text-gradient-airtable">Airtable</span></h2>
      ...
      <div class="flex flex-col justify-center gap-4 sm:flex-row reveal reveal-delay-2">
        <a href="<?php echo esc_url(home_url('/signup')); ?>" class="inline-flex items-center justify-center gap-2 px-10 py-4 text-lg font-semibold transition-all duration-300 rounded-full group bg-airtable-primary text-airtable-dark hover:bg-airtable-secondary hover:text-white hover:scale-105 hover:shadow-glow-airtable">
          <?php echo esc_html( $t['cta_connect'] ); ?>
          <svg class="w-5 h-5 transition-transform group-hover:translate-x-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
        </a>
      ...
      <p class="mt-8 text-sm text-white/30 reveal reveal-delay-3">OAuth 2.0 • Bidirectional Sync • Real-time • Field Mapping • Multiple Bases</p>
    </div>
  </section>
